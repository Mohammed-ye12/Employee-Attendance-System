...
-- Create cron job to clean up old activity history
CREATE OR REPLACE FUNCTION setup_activity_cleanup_job()
RETURNS void AS $$
BEGIN
  -- Run cleanup at midnight on the first day of each month
  PERFORM cron.schedule(
    'cleanup-activity-history',
    '0 0 1 * *',  -- At 00:00 on day-of-month 1
    $$SELECT cleanup_old_activity_history()$$
  );
END;
$$ LANGUAGE plpgsql;

-- Schedule the initial cleanup job
SELECT setup_activity_cleanup_job();